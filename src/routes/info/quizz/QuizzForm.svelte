<script>
    ...
    function submitAnswers() {
      ...
      if (score <= 5 ){
        resultat.text = 'infos taxecarbone tombe à pic pour vous informer';
      }
      else if ( score <= 8 ){
        resultat.text = 'vous avez des connaissances solides sur le sujet!';
      }
      else {
        resultat.text = 'il n\'y a plus de secret pour vous, nous avons hâte de connaître vos positions!'
      }
    }
...
<main class='flex items-center justify-center h-screen pt-10 pb-10 background10'>
  ...
  <div class="flex flex-col justify-between bg-white rounded-lg shadow-md p-6 w-96 h-96">
      ...
      {:else}
      <h2 class="text-xl font-bold mb-2">Votre score  est de {resultat.score}/{questions.length}</h2>
        <h2 class="text-xl font-bold mb-2">{resultat.text}</h2>
        <div class="relative inset-x-0 bottom-0 h-auto">
          <a href="#" on:click={goBack} class="text-lg bg-blue-100  hover:bg-blue-600 text-gray-700 font-bold py-2 px-4 rounded block text-center">VOIR LES INFOS</a>
        </div>
        {/if}
  </div>
</main>
